...
SOURCES_COMMON := src/main.cpp src/uci.cpp src/util/split.cpp src/position/position.cpp src/movegen.cpp src/search.cpp src/util/timer.cpp src/pretty.cpp src/ttable.cpp src/limit/time.cpp src/eval/nnue.cpp src/perft.cpp src/bench.cpp src/tunable.cpp src/opts.cpp src/3rdparty/fathom/tbprobe.cpp src/datagen/datagen.cpp src/wdl.cpp src/cuckoo.cpp src/datagen/marlinformat.cpp src/datagen/viri_binpack.cpp
...
CXXFLAGS := -std=c++20 -O3 -flto -DNDEBUG -DSPJ_NETWORK_FILE=\"$(EVALFILE)\" -DSPJ_VERSION=$(VERSION) -D_SILENCE_CXX20_ATOMIC_INIT_DEPRECATION_WARNING

CXXFLAGS_NATIVE := -DSPJ_NATIVE -march=native
CXXFLAGS_TUNABLE := -DSPJ_NATIVE -march=native -DSPJ_EXTERNAL_TUNE=1
CXXFLAGS_AVX512 := -DSPJ_AVX512 -DSPJ_FAST_PEXT -march=x86-64-v4 -mtune=znver4
CXXFLAGS_AVX2_BMI2 := -DSPJ_AVX2_BMI2 -DSPJ_FAST_PEXT -march=haswell -mtune=haswell
...
ifneq (, $(findstring clang,$(COMPILER_VERSION)))
    ...
    PGO_GENERATE := -DSPJ_PGO_PROFILE -fprofile-instr-generate
    ...
    ifeq ($(findstring __znver1, $(ARCH_DEFINES)),)
        ifeq ($(findstring __znver2, $(ARCH_DEFINES)),)
            ifeq ($(findstring __bdver, $(ARCH_DEFINES)),)
                CXXFLAGS_NATIVE += -DSPJ_FAST_PEXT
            endif
        endif
    endif
endif
...
ifeq ($(COMMIT_HASH),on)
    CXXFLAGS += -DSPJ_COMMIT_HASH=$(shell git log -1 --pretty=format:%h)
endif

PROFILE_OUT = SPJ_profile$(SUFFIX)
